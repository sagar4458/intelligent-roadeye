...
│   ├── train.py            # VGG16 training pipeline (PyTorch)
│   ├── predict.py          # Inference engine
│   ├── roadeye_vgg16.pth   # Trained model weights
│   └── training_history.png
├── frontend/
│   └── index.html          # Dashboard UI
├── data/
│   └── merged/             # Balanced dataset (generated by merge_dataset.py)
│       ├── crack/
│       └── pothole/
...
The original 2024 version was built on TensorFlow 2.10. When rebuilding in 2026, TF 2.10 on Windows with driver 581.83 and RTX 3050 throws `CUDA_ERROR_NOT_PERMITTED` and falls back to CPU silently. PyTorch 2.7 with CUDA 11.8 works correctly on the same machine — training completed in under 5 minutes instead of 40+. The switch was a practical decision, not a preference.
